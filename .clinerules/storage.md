...
Global settings, secrets and workspace state are stored in **file-backed JSON stores** under `~/.enki/data/`. This is the shared storage layer used by VSCode, CLI, and JetBrains.

## Key Abstractions

### `StorageContext` (src/shared/storage/storage-context.ts)
The entry point. Created via `createStorageContext()` and passed to `StateManager.initialize()`. Contains three `Enki AIFileStorage` instances:
- `globalState` → `~/.enki/data/globalState.json`
- `secrets` → `~/.enki/data/secrets.json` (mode 0o600)
- `workspaceState` → `~/.enki/data/workspaces/<hash>/workspaceState.json`

### `Enki AIFileStorage` (src/shared/storage/Enki AIFileStorage.ts)
...
Remember that your data may be read by a different client than the one that wrote it. For example, a value written by Enki AI in JetBrains may be read by Enki AI CLI.
