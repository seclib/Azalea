# Enki AI Extension Architecture & Development Guide

## Project Overview

Enki AI is a VSCode extension that provides AI assistance through a combination of a core extension backend and a React-based webview frontend. The extension is built with TypeScript and follows a modular architecture pattern.
...
The WebviewProvider maintains a reference to the Controller and delegates message handling to it. It also handles the creation of both sidebar and tab panel webviews, allowing Enki AI to be used in different contexts within VSCode.
...
Enki AI supports multiple AI providers through a modular API provider system. Each provider is implemented as a separate module in the `src/api/providers/` directory and follows a common interface.
...
Enki AI supports separate model configurations for Plan and Act modes:
...
class Task {
  async *attemptApiRequest(previousApiReqIndex: number): ApiStream {
    ...
    const previousRequest = this.enkiMessages[previousApiReqIndex]
...
class Task {
  async resumeTaskFromHistory() {
    // 1. Load saved state
    this.enkiMessages = await getSavedEnki AIMessages(this.getContext(), this.taskId)
  ...
  private async saveTaskState() {
    ...
    await saveEnki AIMessages(this.getContext(), this.taskId, this.enkiMessages)
...
Enki AI implements a dual-mode system that separates planning from execution:
...
Enki AI supports two types of MCP server connections:
...
class Controller {
  ...
  async downloadMcp(mcpId: string) {
    ...
    const response = await axios.post<McpDownloadResponse>(
      "https://api.enki.bot/v1/mcp/download",
      { mcpId },
      {
    ...
    await this.initEnki AIWithTask(task)
  }
}
```
...
This guide provides a comprehensive overview of the Enki AI extension architecture, with special focus on state management, data persistence, and code organization. Following these patterns ensures robust feature implementation with proper state handling across the extension's components.
...
Contributions to the Enki AI extension are welcome! Please follow these guidelines:
...
The `.enkiignore` file allows users to specify files and directories that Enki AI should not access. When implementing new features, respect the `.enkiignore` rules and ensure that your code does not attempt to read or modify ignored files.
