...
Full end-to-end tests using real-world tasks from enki-bench.

## Overview

These tests run Enki AI against production-grade coding problems derived from actual user sessions. Each task:
- Starts with a broken codebase in Docker
- Gives Enki AI the task description
...
npx tsx evals/e2e/run-enki-bench.ts

# Run specific task
npx tsx evals/e2e/run-enki-bench.ts --tasks discord

# Use different provider/model
npx tsx evals/e2e/run-enki-bench.ts --provider openai --model gpt-4o

# Run on Daytona cloud (faster, parallel)
export DAYTONA_API_KEY=dtn_...
npx tsx evals/e2e/run-enki-bench.ts --env daytona

# Output to JSON
npx tsx evals/e2e/run-enki-bench.ts --output results.json
```
...
Current tasks from enki-bench (12 total):
...
Results are written to `evals/enki-bench/jobs/` directory by Harbor:
...
└── 2025-01-25__10-00-00/
    ...
    └── <task-id>__<hash>/
        ...
        ├── agent/enki.txt      # Conversation log
