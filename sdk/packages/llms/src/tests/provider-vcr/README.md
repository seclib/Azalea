...
These cassettes exercise the real provider adapters in `@enki/llms` without
requiring credentials during normal test runs.

Run playback from `sdk/`:

```sh
bun -F @enki/llms test:vcr
```

Refresh the cassettes from local provider credentials:

```sh
LLMS_PROVIDER_VCR_RECORD=1 bun -F @enki/llms test:vcr
```

Refresh one cassette by provider id or cassette name:

```sh
LLMS_PROVIDER_VCR_RECORD=1 LLMS_PROVIDER_VCR_TARGET=enki bun -F @enki/llms test:vcr
```

Record mode prefers the normal Enki AI CLI provider settings path. To use another
...
ChatGPT OAuth needs saved `openai-codex` provider settings. For local Enki AI API
