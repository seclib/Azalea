...
export type { Enki AICoreSettingsApi } from "../settings";
...
export interface Enki AICoreAutomationOptions {
...
export type Enki AIAutomationSpec = CronSpecRecord;
export type Enki AIAutomationRun = CronRunRecord;
export type Enki AIAutomationEventLog = CronEventLogRecord;
export type Enki AIAutomationEventSuppression = CronEventSuppression;
export type Enki AIAutomationRunStatus =
	| "queued"
	| "running"
	| "done"
	| "failed"
...
export interface Enki AIAutomationListSpecsOptions {
...
export interface Enki AIAutomationListRunsOptions {
	specId?: string;
	status?: Enki AIAutomationRunStatus | Enki AIAutomationRunStatus[];
	limit?: number;
}

export interface Enki AIAutomationListEventsOptions {
	eventType?: string;
	source?: string;
	processingStatus?:
		| "received"
...
export interface Enki AIAutomationEventIngressResult {
	event: Enki AIAutomationEventLog;
	duplicate: boolean;
	matchedSpecIds: string[];
	queuedRuns: Enki AIAutomationRun[];
	suppressions: Enki AIAutomationEventSuppression[];
}

export interface Enki AICoreAutomationApi {
	start(): Promise<void>;
	stop(): Promise<void>;
	...
	listEvents(
		options?: Enki AIAutomationListEventsOptions,
	): Enki AIAutomationEventLog[];
	getEvent(eventId: string): Enki AIAutomationEventLog | undefined;
	listSpecs(options?: Enki AIAutomationListSpecsOptions): Enki AIAutomationSpec[];
	listRuns(options?: Enki AIAutomationListRunsOptions): Enki AIAutomationRun[];
}

export type Enki AICoreListHistoryOptions = SessionHistoryListOptions;

export interface Enki AICoreStartInput
...
export interface Enki AICoreOptions {
	/**
	...
	/**
	 * Feature flags service for this Enki AICore instance.
	...
	/**
	 * Enables file-based and event-driven automation through this Enki AICore
	 * instance. When configured, callers use `enki.automation.*` instead of
	 * constructing cron services directly.
	 */
	automation?: boolean | Enki AICoreAutomationOptions;
	/**
	 ...
	 *     `@enki/core/hub` for the scheduler
	 */
	fetch?: typeof fetch;
	/**
...
export interface StartSessionBootstrap {
	applyToStartSessionInput(
		input: Enki AICoreStartInput,
	): Promise<Enki AICoreStartInput> | Enki AICoreStartInput;
