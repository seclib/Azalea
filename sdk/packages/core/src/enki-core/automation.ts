...
export function normalizeAutomationOptions(
	options: Enki AICoreAutomationOptions | boolean | undefined,
): Enki AICoreAutomationOptions | undefined {
...
export function normalizeAutomationCronScope(
	scope: Enki AICoreAutomationOptions["cronScope"],
): "global" | "workspace" | undefined {
	if (scope === "user") return "global";
	return scope;
}

export class Enki AICoreAutomationController implements Enki AICoreAutomationApi {
...
export interface Enki AICoreAutomationRuntimeHandlersInput {
	host: RuntimeHost;
	getExtensionContext(): ExtensionContext | undefined;
}

export function createEnki AICoreAutomationRuntimeHandlers(
	input: Enki AICoreAutomationRuntimeHandlersInput,
	...
	return {
		...
		async sendSession(sessionId, request) {
			...
			if (!result) {
				throw new Error("Enki AICore automation runtime returned no result");
			}
			return { result: toChatTurnResult(result) };
		},
		async abortSession(sessionId) {
			await host.abort(sessionId, new Error("Enki AICore automation abort"));
			return { applied: true };
		},
...
export interface Enki AICoreAutomationExtensionContextInput {
	automationService?: CronService;
	automation: Enki AICoreAutomationApi;
...
export function createEnki AICoreAutomationExtensionContext(
	input: Enki AICoreAutomationExtensionContextInput,
