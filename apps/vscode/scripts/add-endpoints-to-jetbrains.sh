...
if [ "$#" -ne 3 ]; then
    ...
    echo ""
    echo "Example:"
    echo "  $0 enki-jetbrains-3.55.0.zip enki-jetbrains-3.55.0-enterprise.zip endpoints.json"
    exit 1
fi
...
# JetBrains plugins typically have a structure like: enki/lib/
# We need to add endpoints.json to the root of the plugin directory
PLUGIN_DIR="$TEMP_DIR/enki"
...
echo "When installed in JetBrains IDEs, Enki AI will automatically use the bundled configuration."
