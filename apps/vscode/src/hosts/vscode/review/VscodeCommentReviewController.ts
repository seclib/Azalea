...
export class VscodeCommentReviewController extends CommentReviewController implements vscode.Disposable {
	...
	constructor() {
		...
		this.commentController = vscode.comments.createCommentController("enki-ai-review", "Enki AI AI Review")

		// Configure options for the reply input
		this.commentController.options = {
			placeHolder: "Ask a question about this code...",
			prompt: "Reply to Enki AI",
		}
		...
		this.disposables.push(
			vscode.commands.registerCommand("enki.reviewComment.reply", async (reply: vscode.CommentReply) => {
				await this.handleReply(reply)
			}),
		)

		// Register add to chat command - sends the conversation to Enki AI's main chat
		this.disposables.push(
			vscode.commands.registerCommand("enki.reviewComment.addToChat", async (thread: vscode.CommentThread) => {
	...
	/**
	 * Handle adding the thread conversation to Enki AI's main chat
	 */
...
Please continue helping the user with their question about this code.`
	...
	/**
	 * Close all tabs that use the enki-diff URI scheme (both diff views and regular text documents)
	 */
	async closeDiffViews(): Promise<void> {
		const tabs = vscode.window.tabGroups.all
			...
			.filter((tab) => {
				...
				// Check for regular text document tabs with enki-diff scheme (opened during comment reveal)
