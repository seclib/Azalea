...
export async function validateWorkspacePath(workspacePath: string): Promise<void> {
	...
	try {
		const testFile = path.join(workspacePath, ".enki_write_test")
...
export async function initializeGitRepository(workspacePath: string): Promise<boolean> {
	...
	try {
		...
		await execa("git", ["config", "user.name", "Enki AI Evaluation"], { cwd: workspacePath })
		await execa("git", ["config", "user.email", "enki@example.com"], { cwd: workspacePath })
