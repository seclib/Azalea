...
export type TelemetryProviderConfig =
	 ...
	 * @param bypassUserSettings - When true, telemetry is sent regardless of the user's Enki AI telemetry opt-in/opt-out settings.
	 * This is used for:
...
export class TelemetryProviderFactory {
	...
	public static getDefaultConfigs(): TelemetryProviderConfig[] {
		...
		if (!Enki AIEndpoint.isSelfHosted() && isPostHogConfigValid(posthogConfig)) {
			configs.push({ type: "posthog", ...posthogConfig })
		}

		// Skip build-time OTEL in selfHosted mode - enterprise customers should not send telemetry to Enki AI's collector
		...
		if (!Enki AIEndpoint.isSelfHosted() && otelConfig) {
			configs.push({
				type: "opentelemetry",
				config: otelConfig,
		...
		if (runtimeOtelConfig) {
			configs.push({
				...
				// OTEL regardless of his Enki AI telemetry settings
