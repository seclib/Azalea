...
export function registerTaskCommands(controller: Controller): vscode.Disposable[] {
	return [
		vscode.commands.registerCommand("enki.dev.expireMcpOAuthTokens", async () => {
			try {
		...
		vscode.commands.registerCommand("enki.dev.createTestTasks", async () => {
			const count = (
 ...
 */
function createRealisticMessageSequence(baseTimestamp: number, taskPrompt: string, taskIndex: number): Enki AIMessage[] {
	...
	const messages: Enki AIMessage[] = [
		// Initial task message - uses "say" with "text" which is the format used in Enki AI.ts
		{
			ts: baseTimestamp,
			type: "say",
			say: "text",
