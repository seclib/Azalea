...
describe("Enki AI Extension", () => {
	...
	it("should verify extension ID matches package.json", async () => {
		...
		const enkiExtensionApi = vscode.extensions.getExtension(id)

		enkiExtensionApi?.id.should.equal(id)
	})

	it("should successfully execute the plus button command", async () => {
		await new Promise((resolve) => setTimeout(resolve, 400))
		await vscode.commands.executeCommand("enki.plusButtonClicked")
	})
