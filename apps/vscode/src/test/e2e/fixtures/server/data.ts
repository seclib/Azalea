...
export class Enki AIDataMock {
	...
	public switchToUserType(type: "personal" | "enterprise"): UserResponse {
		const token = Enki AIDataMock.getDefaultToken(type)
		if (!token) {
	...
	private _createUserResponse(userData: (typeof Enki AIDataMock.USERS)[0]): UserResponse {
	...
	public getUserByToken(token?: string): UserResponse {
		...
		const actualToken = token || Enki AIDataMock.getDefaultToken("personal") || "test-personal-token"
	...
	private _getUserByToken(token: string): UserResponse {
		const match = Enki AIDataMock.findUserByToken(token)
