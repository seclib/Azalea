 ...
 * Mirrors the Enki AISayHook interface but represents parsed data.
 */
...
function getSay(msg: Enki AIMessage): string | undefined {
 ...
 */
function isToolOrCommandMessage(msg: Enki AIMessage): boolean {
 ...
 */
function parseHookMetadata(hookMessage: Enki AIMessage): HookMetadata | null {
 ...
 */
function dedupeToolOrCommandMessagesByTimestamp(messages: Enki AIMessage[]): Enki AIMessage[] {
...
function combineHookWithOutputs(
	hookMessage: Enki AIMessage,
	startIndex: number,
	messages: Enki AIMessage[],
): { combined: Enki AIMessage; nextIndex: number } {
 ...
 */
function combineAllHooks(messages: Enki AIMessage[]): Enki AIMessage[] {
	// Pass 1: Build map of combined hooks by timestamp
	const combinedHooksByTs = new Map<number, Enki AIMessage>()
 ...
 */
function findImmediateNextToolTimestamp(hookIndex: number, messages: Enki AIMessage[]): number | null {
 ...
 */
function buildPreToolUseMap(processedMessages: Enki AIMessage[], originalMessages: Enki AIMessage[]): Map<number, Enki AIMessage[]> {
	const map = new Map<number, Enki AIMessage[]>()
 ...
 */
function reorderWithPreToolUseHooks(messages: Enki AIMessage[], preToolUseMap: Map<number, Enki AIMessage[]>): Enki AIMessage[] {
	const result: Enki AIMessage[] = []
 ...
 *
 * @param messages Array of Enki AIMessage objects to process
 * @returns New array with hooks combined and PreToolUse hooks reordered
 */
export function combineHookSequences(messages: Enki AIMessage[]): Enki AIMessage[] {
