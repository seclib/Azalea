import { getSavedEnki AIMessages, getTaskMetadata, readTaskHistoryFromState, writeTaskHistoryToState } from "@core/storage/disk"
...
async function reconstructTaskHistoryItem(taskId: string): Promise<HistoryItem | null> {
	try {
		...
		const enkiMessages = await getSavedEnki AIMessages(taskId)
		if (enkiMessages.length === 0) {
		...
		const taskInfo = extractTaskInformation(enkiMessages, metadata)
...
function extractTaskInformation(enkiMessages: Enki AIMessage[], metadata: any): TaskInfo {
	// Find the first user message (task description)
	const firstUserMessage = enkiMessages.find((msg) => msg.type === "say" && msg.say === "text" && msg.text)

	// Extract timestamp from first message or use task ID as fallback
	const timestamp = enkiMessages.length > 0 ? enkiMessages[0].ts : Date.now()
	...
	const apiReqMessages = enkiMessages.filter(
	...
	const messageSize = JSON.stringify(enkiMessages).length
