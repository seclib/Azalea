...
export class ReportBugHandler implements IToolHandler, IPartialBlockHandler {
	readonly name = Enki AIDefaultTool.REPORT_BUG
	...
	async execute(config: TaskConfig, block: ToolUse): Promise<ToolResponse> {
		...
		if (config.autoApprovalSettings.enableNotifications) {
			showSystemNotification({
				subtitle: "Enki AI wants to create a github issue...",
				message: `Enki AI is suggesting to create a github issue with the title: ${title}`,
			})
		}
		...
		const enkiVersion = ExtensionRegistryInfo.version
		...
		if (text || (images && images.length > 0) || (reportBugFiles && reportBugFiles.length > 0)) {
			...
			try {
				...
				params.set("enki-version", enkiVersion)
				...
				await createAndOpenGitHubIssue("enki", "enki", "bug_report.yml", params)
			} catch (error) {
