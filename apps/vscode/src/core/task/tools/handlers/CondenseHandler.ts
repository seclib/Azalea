...
export class CondenseHandler implements IToolHandler, IPartialBlockHandler {
	readonly name = Enki AIDefaultTool.CONDENSE
	...
	async execute(config: TaskConfig, block: ToolUse): Promise<ToolResponse> {
		...
		if (config.autoApprovalSettings.enableNotifications) {
			showSystemNotification({
				subtitle: "Enki AI wants to condense the conversation...",
				message: `Enki AI is suggesting to condense your conversation with: ${context}`,
			})
		}
	...
	async handlePartialBlock(block: ToolUse, uiHelpers: StronglyTypedUIHelpers): Promise<void> {
		...
		await uiHelpers.ask("condense" as Enki AIAsk, cleanedContext, block.partial).catch(() => {})
	}
}
