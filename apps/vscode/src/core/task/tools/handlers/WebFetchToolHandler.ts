import { Enki AIAsk, Enki AISayTool } from "@shared/ExtensionMessage"
...
export class WebFetchToolHandler implements IFullyManagedTool {
	readonly name = Enki AIDefaultTool.WEB_FETCH
	...
	async handlePartialBlock(block: ToolUse, uiHelpers: StronglyTypedUIHelpers): Promise<void> {
		...
		await uiHelpers.ask("tool" as Enki AIAsk, partialMessage, block.partial).catch(() => {})
	}
	...
	async execute(config: TaskConfig, block: ToolUse): Promise<ToolResponse> {
		try {
			...
			// Check if Enki AI web tools are enabled (both user setting and feature flag)
			const enkiWebToolsEnabled = config.services.stateManager.getGlobalSettingsKey("enkiWebToolsEnabled")
			const featureFlagEnabled = featureFlagsService.getWebtoolsEnabled()
			if (provider !== "enki" || !enkiWebToolsEnabled || !featureFlagEnabled) {
				return formatResponse.toolError("Enki AI web tools are currently disabled.")
			}
			...
			if (config.callbacks.shouldAutoApproveTool(this.name)) {
				...
				showNotificationForApproval(
					`Enki AI wants to fetch content from ${url}`,
			...
			const baseUrl = Enki AIEnv.config().apiBaseUrl
			...
			const response = await axios.post(
				...
				},
				{
					headers: {
						...
						...(await buildEnki AIExtraHeaders()),
					},
					timeout: 15000,
					...getAxiosSettings(),
				},
