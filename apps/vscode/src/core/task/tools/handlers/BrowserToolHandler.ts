import { BrowserAction, BrowserActionResult, browserActions, Enki AISayBrowserAction } from "@shared/ExtensionMessage"
...
export class BrowserToolHandler implements IFullyManagedTool {
	readonly name = Enki AIDefaultTool.BROWSER
	...
	async handlePartialBlock(block: ToolUse, uiHelpers: StronglyTypedUIHelpers): Promise<void> {
		...
		if (action === "launch") {
			...
			await uiHelpers.say(
				...
				} satisfies Enki AISayBrowserAction),
				undefined,
				undefined,
				block.partial,
			)
			...
			if (action === "launch") {
				...
				if (autoApprover.shouldAutoApproveTool(block.name)) {
					...
					showNotificationForApproval(
						`Enki AI wants to use a browser and launch ${url}`,
