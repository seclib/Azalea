...
import type { Enki AISayGenerateExplanation } from "@/shared/ExtensionMessage"
...
/**
 * Helper to create a stringified Enki AISayGenerateExplanation message
 */
function createExplanationMessage(
	title: string,
	fromRef: string,
	toRef: string,
	status: Enki AISayGenerateExplanation["status"],
	error?: string,
): string {
	const message: Enki AISayGenerateExplanation = { title, fromRef, toRef, status }
	if (error) {
		message.error = error
	}
...
export class GenerateExplanationToolHandler implements IToolHandler, IPartialBlockHandler {
	readonly name = Enki AIDefaultTool.GENERATE_EXPLANATION
