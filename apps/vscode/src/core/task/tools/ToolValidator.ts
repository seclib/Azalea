...
export class ToolValidator {
	constructor(private readonly enkiIgnoreController: Enki AIIgnoreController) {}
	...
	/**
	 * Verifies access is allowed to a given path via .enkiignore rules.
	 * Callers should pass a repo-relative (workspace-relative) path.
	 */
	checkEnki AIIgnorePath(relPath: string): ValidationResult {
		const accessAllowed = this.enkiIgnoreController.validateAccess(relPath)
		if (!accessAllowed) {
			return {
				ok: false,
				error: `Access to path '${relPath}' is blocked by .enkiignore settings.`,
			}
		}
		return { ok: true }
	}
