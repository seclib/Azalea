...
/**
 * Builds an array of Enki AIContent blocks from user feedback inputs.
 ...
 * @returns Array of Enki AIContent blocks ready for hook processing (may be empty if no content provided)
 */
export async function buildUserFeedbackContent(text?: string, images?: string[], files?: string[]): Promise<Enki AIContent[]> {
	const content: Enki AIContent[] = []
