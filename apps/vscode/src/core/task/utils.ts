...
import { Enki AIApiReqCancelReason, Enki AIApiReqInfo } from "@/shared/ExtensionMessage"
...
export const updateApiReqMsg = async (params: UpdateApiReqMsgParams) => {
	const enkiMessages = params.messageStateHandler.getEnki AIMessages()
	const currentApiReqInfo: Enki AIApiReqInfo = JSON.parse(enkiMessages[params.lastApiReqIndex].text || "{}")
	delete currentApiReqInfo.retryStatus // Clear retry status when request is finalized

	await params.messageStateHandler.updateEnki AIMessage(params.lastApiReqIndex, {
		text: JSON.stringify({
