...
import { version as enkiVersion } from "../../../package.json"
 ...
 * provide--the other common parameters like enkiVersion and userId are handled by the
 * hook system.
 */
...
export abstract class HookRunner<Name extends HookName> {
	 ...
	 * - enkiVersion: Current Enki AI extension version
	 ...
	 * - userId: Enki AI user ID, machine ID, or generated UUID
	 *
	...
	protected async completeParams(params: NamedHookInput<Name>): Promise<HookInput> {
		...
		return {
			...
			workspaceRoots,
			userId: getDistinctId(), // Always available: Enki AI User ID, machine ID, or generated UUID
			...params,
			model,
		}
	}
 ...
 *
 * Used in multi-root workspaces where both global hooks (from ~/Documents/Enki AI/Hooks/)
 * and workspace-specific hooks (from each workspace's .enkirules/hooks/) exist for the
 * same hook type.
 *
 * Behavior:
...
function isExpectedHookError(error: unknown): boolean {
	...
	// Note: This is expected because users may have hooks in .enkirules that they don't want to execute
...
export class HookFactory {
	 ...
	 * Global hooks are located in paths containing "Enki AI/Hooks" or "enki/hooks".
	 */
	 ...
	 *
	 * - Global hooks (from ~/Documents/Enki AI/Hooks/): run from the primary workspace root
	 * - Workspace hooks (from workspaceRoot/.enkirules/hooks/): run from that specific workspace root
	 *
	...
	private determineHookCwd(
		...
		// Workspace hooks are at: workspaceRoot/.enkirules/hooks/
	 ...
	 * Global hooks are located in ~/Documents/Enki AI/Hooks/
	 * Workspace hooks are located in workspace .enkirules/hooks/ directories
	 *
	 ...
	 * Includes both global hooks (from ~/Documents/Enki AI/Hooks/) and workspace hooks
	 * (from .enkirules/hooks/ in each workspace root).
	 */
	...
	/**
	 * Finds the path to a hook in a .enkirules hooks directory.
	 *
	 * @param hookName the name of the hook to search for, for example 'PreToolUse'
	 * @param hooksDir the .enkirules directory path to search
