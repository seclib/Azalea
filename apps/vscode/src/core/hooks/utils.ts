...
/**
 * All valid hook types that can be created and executed by Enki AI.
...
export async function resolveHooksDirectory(
	...
	if (isGlobal) {
		return globalHooksDirOverride || path.join(os.homedir(), "Documents", "Enki AI", "Hooks")
	}
	...
	if (workspaceName) {
		...
		return path.join(targetWorkspace, ".enkirules", "hooks")
	}

	// Single workspace: use getCwd
	const cwd = await getCwd(getDesktopDir())
	return path.join(cwd, ".enkirules", "hooks")
}
