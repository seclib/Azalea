...
export interface Enki AIToolSpec {
	variant: ModelFamily
	id: Enki AIDefaultTool
	name: string
	description: string
	instruction?: string
	contextRequirements?: (context: SystemPromptContext) => boolean
	parameters?: Array<Enki AIToolSpecParameter>
}

interface Enki AIToolSpecParameter {
	name: string
	required: boolean
	instruction: string | ((context: SystemPromptContext) => string)
	usage?: string
	dependencies?: Enki AIDefaultTool[]
	description?: string
	contextRequirements?: (context: SystemPromptContext) => boolean
...
/**
 * Converts a Enki AIToolSpec into an OpenAI ChatCompletionTool definition
 * Docs: https://openrouter.ai/docs/features/tool-calling#step-1-inference-request-with-tools
 */
export function toolSpecFunctionDefinition(tool: Enki AIToolSpec, context: SystemPromptContext): OpenAITool {
...
/**
 * Converts a Enki AIToolSpec into an Anthropic Tool definition
 */
export function toolSpecInputSchema(tool: Enki AIToolSpec, context: SystemPromptContext): AnthropicTool {
	// Check if the tool should be included based on context requirements
	if (tool.contextRequirements && !tool.contextRequirements(context)) {
		throw new Error(`Tool ${tool.name} does not meet context requirements`)
	}
...
/**
 * Converts a Enki AIToolSpec into a Google Gemini function.
 * Docs: https://ai.google.dev/gemini-api/docs/function-calling
 */
export function toolSpecFunctionDeclarations(tool: Enki AIToolSpec, context: SystemPromptContext): GoogleTool {
	// Check if the tool should be included based on context requirements
	if (tool.contextRequirements && !tool.contextRequirements(context)) {
		throw new Error(`Tool ${tool.name} does not meet context requirements`)
	}
