import { HookInfo, HooksToggles, WorkspaceHooks } from "@shared/proto/enki/file"
...
export async function refreshHooks(
	...
	const globalHooksDir = globalHooksDirOverride || path.join(os.homedir(), "Documents", "Enki AI", "Hooks")
	...
	for (const workspacePath of workspacePaths.paths) {
		const workspaceHooksDir = path.join(workspacePath, ".enkirules", "hooks")
