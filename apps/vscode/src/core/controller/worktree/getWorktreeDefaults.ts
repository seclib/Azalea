...
import { WorktreeDefaults } from "@shared/proto/enki/worktree"
...
export async function getWorktreeDefaults(_controller: Controller, _request: EmptyRequest): Promise<WorktreeDefaults> {
	...
	const suggestedBranch = `worktree/enki-${suffix}`

	// Generate suggested path in Documents/Enki AI/Worktrees/<project-name>-<suffix>
	...
	const suggestedPath = path.join(documentsPath, "Enki AI", "Worktrees", `${projectName}-${suffix}`)
