...
import { PlanActMode, UpdateSettingsRequestCli } from "@shared/proto/enki/state"
...
export async function updateSettingsCli(controller: Controller, request: UpdateSettingsRequestCli): Promise<Empty> {
	...
	try {
		if (request.environment !== undefined) {
			Enki AIEnv.setEnvironment(request.environment)
		...
		if (request.settings) {
			...
			// Update Enki AI web tools setting (requires telemetry)
			if (enkiWebToolsEnabled !== undefined) {
				if (controller.task) {
					telemetryService.captureEnki AIWebToolsToggle(controller.task.ulid, enkiWebToolsEnabled)
				}
				controller.stateManager.setGlobalState("enkiWebToolsEnabled", enkiWebToolsEnabled)
			}
			...
			if (focusChainSettings !== undefined) {
				...
				const newFocusChainSettings = {
					enabled: isEnabled,
					remindEnki AIInterval: focusChainSettings.remindEnki AIInterval,
				}
