...
async function fetchAndCacheModels(controller: Controller): Promise<Record<string, ModelInfo>> {
	...
	const finalModels = appendEnki AIStealthModels(models)
...
export function appendEnki AIStealthModels(currentModels: Record<string, ModelInfo>): Record<string, ModelInfo> {
