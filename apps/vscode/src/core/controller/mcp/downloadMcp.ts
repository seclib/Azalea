...
import { McpDownloadResponse } from "@shared/proto/enki/mcp"
...
export async function downloadMcp(controller: Controller, request: StringRequest): Promise<McpDownloadResponse> {
	try {
		...
		const response = await axios.post<McpDownloadResponse>(
			`${Enki AIEnv.config().mcpBaseUrl}/download`,
			{ mcpId },
			{
...
- Use "${mcpDetails.mcpId}" as the server name in enki_mcp_settings.json.
- Create the directory for the new MCP server before starting installation.
- Make sure you read the user's existing enki_mcp_settings.json file before editing it with this new mcp, to not overwrite any existing servers.
