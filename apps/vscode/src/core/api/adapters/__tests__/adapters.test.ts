...
+++++++ REPLACE`,
		...
		testCases.forEach((testCase) => {
			it(testCase.name, () => {
				...
				const result = transformToolCallMessages(testCase.input, [Enki AIDefaultTool.FILE_EDIT, Enki AIDefaultTool.FILE_NEW])
		...
		it("should preserve tool_result blocks when tool_use is converted", () => {
			const input: Enki AIStorageMessage[] = [
				{
					role: "assistant",
					content: [
						{
...
EOF`,
			...
			// Native tools are FILE_EDIT/FILE_NEW, messages use apply_patch → convert FROM apply_patch
			const result = transformToolCallMessages(input, [Enki AIDefaultTool.FILE_EDIT, Enki AIDefaultTool.FILE_NEW])
...
EOF`,
		...
		testCases.forEach((testCase) => {
			it(testCase.name, () => {
				...
				const result = transformToolCallMessages(testCase.input, [Enki AIDefaultTool.APPLY_PATCH])
		...
		it("should use final_file_content to generate apply_patch for write_to_file", () => {
			...
			// Native tools are APPLY_PATCH, messages use write_to_file/replace_in_file → convert TO apply_patch
			const result = transformToolCallMessages(input, [Enki AIDefaultTool.APPLY_PATCH])
