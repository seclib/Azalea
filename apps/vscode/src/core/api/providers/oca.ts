...
import { ApiFormat } from "@/shared/proto/index.enki"
...
export class OcaHandler implements ApiHandler {
	...
	async *createMessageChatApi(systemPrompt: string, messages: Enki AIStorageMessage[], tools?: OpenAITool[]): ApiStream {
		const client = this.ensureOpenAIClient()
		...
		const chatCompletionsParams: OpenAI.Chat.Completions.ChatCompletionCreateParamsStreaming = {
			...
			...(this.options.taskId && {
				litellm_session_id: `enki-${this.options.taskId}`,
	...
	async *createMessageResponsesApi(systemPrompt: string, messages: Enki AIStorageMessage[], tools?: OpenAITool[]): ApiStream {
		const client = this.ensureOpenAIClient()
	...
	async *createMessageMessagesApi(systemPrompt: string, messages: Enki AIStorageMessage[], tools?: OpenAITool[]): ApiStream {
