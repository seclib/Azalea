...
describe("FileContextTracker", () => {
	...
	it("should add a record when a file is read by a tool", async () => {
		...
		expect(fileEntry.enki_read_date).to.be.a("number")
		expect(fileEntry.enki_edit_date).to.be.null
	})

	it("should add a record when a file is edited by Enki AI", async () => {
		await tracker.trackFileContext(filePath, "enki_edited")
		...
		expect(activeEntry.record_source).to.equal("enki_edited")
		expect(activeEntry.enki_read_date).to.be.a("number")
		expect(activeEntry.enki_edit_date).to.be.a("number")
	})
	...
	it("should mark existing entries as stale when adding a new entry for the same file", async () => {
		...
		mockTaskMetadata.files_in_context = [
			{
				...
				enki_read_date: Date.now() - 1000, // 1 second ago
				enki_edit_date: null,
				user_edit_date: null,
			},
		]
		...
		expect(newEntry.record_source).to.equal("enki_edited")
	})
	...
	it("should not track Enki AI edits as user edits", async () => {
		// First track the file to set up the watcher
		await tracker.trackFileContext(filePath, "read_tool")

		// Mark the file as edited by Enki AI
		tracker.markFileAsEditedByEnki AI(filePath)
