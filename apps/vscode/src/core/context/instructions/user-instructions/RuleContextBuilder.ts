...
type MessageStateHandlerLike = {
	getEnki AIMessages(): Enki AIMessageLike[]
}
...
/**
 * Builds the evaluation context used for conditional Enki AI Rules (e.g. YAML frontmatter `paths:`).
 *
 * Kept in the user-instructions domain so Task remains orchestration-focused.
 *
 * Path context is gathered from multiple sources in enkiMessages:
...
export class RuleContextBuilder {
	...
	private static async getRulePathContext(deps: RuleContextBuilderDeps): Promise<string[]> {
		...
		const enkiMessages = deps.messageStateHandler.getEnki AIMessages()
		...
		const lastUserMsg = [...enkiMessages]
			.reverse()
		...
		// (3) Files edited by Enki AI during this task (completed operations):
