...
import { Enki AIConfigurationError, Enki AIEndpoint, Enki AIEnv, Environment } from "../config"

describe("Enki AIEndpoint configuration", () => {
	...
	beforeEach(async () => {
		...
		// Create .enki directory
		await fs.mkdir(path.join(tempDir, ".enki"), { recursive: true })
		...
		;(Enki AIEndpoint as any)._instance = null
		;(Enki AIEndpoint as any)._initialized = false
		;(Enki AIEndpoint as any)._extensionFsPath = undefined
	})
	...
	describe("valid config parsing", () => {
		it("should parse valid endpoints.json with all required fields", async () => {
			...
			await fs.writeFile(path.join(tempDir, ".enki", "endpoints.json"), JSON.stringify(validConfig), "utf8")

			await Enki AIEndpoint.initialize(tempDir)

			const config = Enki AIEndpoint.config
		...
		it("should work without endpoints.json (standard mode)", async () => {
			...
			config.appBaseUrl.should.equal("https://app.enki.bot")
			config.apiBaseUrl.should.equal("https://api.enki.bot")
		})
		...
		it("should accept URLs with paths", async () => {
			const validConfig = {
				...
				apiBaseUrl: "https://proxy.enterprise.com/enki/api",
				mcpBaseUrl: "https://proxy.enterprise.com/enki/mcp",
			}
			...
			const config = Enki AIEndpoint.config
			config.appBaseUrl.should.equal("https://proxy.enterprise.com/enki/app")
		})
	})

	describe("invalid JSON handling", () => {
		it("should throw Enki AIConfigurationError for invalid JSON syntax", async () => {
			await fs.writeFile(path.join(tempDir, ".enki", "endpoints.json"), "{ invalid json }", "utf8")

			try {
				await Enki AIEndpoint.initialize(tempDir)
				throw new Error("Should have thrown")
			} catch (error: any) {
				error.should.be.instanceof(Enki AIConfigurationError)
				error.message.should.containEql("Invalid JSON")
			}
		})

		it("should throw Enki AIConfigurationError for truncated JSON", async () => {
			await fs.writeFile(path.join(tempDir, ".enki", "endpoints.json"), '{"appBaseUrl": "https://test.com"', "utf8")
		...
		it("should throw Enki AIConfigurationError for empty file", async () => {
			await fs.writeFile(path.join(tempDir, ".enki", "endpoints.json"), "", "utf8")
		...
		it("should throw Enki AIConfigurationError for non-object JSON", async () => {
			await fs.writeFile(path.join(tempDir, ".enki", "endpoints.json"), '"just a string"', "utf8")
		...
		it("should throw Enki AIConfigurationError for array JSON", async () => {
			await fs.writeFile(path.join(tempDir, ".enki", "endpoints.json"), "[]", "utf8")
		...
		it("should throw Enki AIConfigurationError for null JSON", async () => {
			await fs.writeFile(path.join(tempDir, ".enki", "endpoints.json"), "null", "utf8")
	...
	describe("missing required fields", () => {
		it("should throw Enki AIConfigurationError when appBaseUrl is missing", async () => {
			const config = {
				apiBaseUrl: "https://api.enterprise.com",
				mcpBaseUrl: "https://mcp.enterprise.com",
			}

			await fs.writeFile(path.join(tempDir, ".enki", "endpoints.json"), JSON.stringify(config), "utf8")
		...
		it("should throw Enki AIConfigurationError when apiBaseUrl is missing", async () => {
			const config = {
				appBaseUrl: "https://app.enterprise.com",
				mcpBaseUrl: "https://mcp.enterprise.com",
			}
		...
		it("should throw Enki AIConfigurationError when mcpBaseUrl is missing", async () => {
			const config = {
				appBaseUrl: "https://app.enterprise.com",
				apiBaseUrl: "https://api.enterprise.com",
			}
		...
		it("should throw Enki AIConfigurationError when all fields are missing", async () => {
			await fs.writeFile(path.join(tempDir, ".enki", "endpoints.json"), "{}", "utf8")
		...
		it("should throw Enki AIConfigurationError when field is null", async () => {
			const config = {
				appBaseUrl: null,
				apiBaseUrl: "https://api.enterprise.com",
				mcpBaseUrl: "https://mcp.enterprise.com",
		...
		it("should throw Enki AIConfigurationError when field is empty string", async () => {
			const config = {
				appBaseUrl: "",
				apiBaseUrl: "https://api.enterprise.com",
				mcpBaseUrl: "https://mcp.enterprise.com",
		...
		it("should throw Enki AIConfigurationError when field is whitespace only", async () => {
			const config = {
				appBaseUrl: "   ",
				apiBaseUrl: "https://api.enterprise.com",
				mcpBaseUrl: "https://mcp.enterprise.com",
		...
		it("should throw Enki AIConfigurationError when field is non-string", async () => {
			const config = {
				appBaseUrl: 12345,
				apiBaseUrl: "https://api.enterprise.com",
				mcpBaseUrl: "https://mcp.enterprise.com",
	...
	describe("invalid URL detection", () => {
		it("should throw Enki AIConfigurationError for invalid URL format", async () => {
			const config = {
		...
		it("should throw Enki AIConfigurationError for URL without protocol", async () => {
			const config = {
		...
		it("should throw Enki AIConfigurationError for malformed URL", async () => {
			const config = {
				appBaseUrl: "https://",
				apiBaseUrl: "https://api.enterprise.com",
				mcpBaseUrl: "https://mcp.enterprise.com",
	...
	describe("environment switching blocked in self-hosted mode", () => {
		it("should throw error when trying to change environment in self-hosted mode", async () => {
			...
			Enki AIEndpoint.config.environment.should.equal(Environment.selfHosted)

			// Try to change environment - should throw
			try {
				Enki AIEnv.setEnvironment("staging")
				throw new Error("Should have thrown")
			} catch (error: any) {
		...
		it("should allow environment switching in standard mode", async () => {
			...
			Enki AIEndpoint.config.environment.should.not.equal(Environment.selfHosted)

			// Should be able to change environment
			Enki AIEnv.setEnvironment("staging")
			Enki AIEnv.getEnvironment().environment.should.equal("staging")

			Enki AIEnv.setEnvironment("local")
			Enki AIEnv.getEnvironment().environment.should.equal("local")

			Enki AIEnv.setEnvironment("production")
			Enki AIEnv.getEnvironment().environment.should.equal("production")
		})
	})
	...
	describe("self-hosted mode behavior", () => {
		it("should report selfHosted environment in self-hosted mode", async () => {
			...
			const envConfig = Enki AIEndpoint.config
		...
		it("should use custom endpoints from file", async () => {
			...
			await fs.writeFile(path.join(tempDir, ".enki", "endpoints.json"), JSON.stringify(customConfig), "utf8")
	...
	describe("initialization behavior", () => {
		it("should only initialize once", async () => {
			await Enki AIEndpoint.initialize(tempDir)
			Enki AIEndpoint.isInitialized().should.be.true()
		...
		it("should throw error when accessing config before initialization", async () => {
			...
			try {
				const _ = Enki AIEndpoint.config
				throw new Error("Should have thrown")
			} catch (error: any) {
	...
	describe("isSelfHosted() method", () => {
		it("should return true when not initialized (safety fallback)", async () => {
			...
			Enki AIEndpoint.isInitialized().should.be.false()
			Enki AIEndpoint.isSelfHosted().should.be.true()
		})
		...
		it("should return false when in normal mode (no endpoints.json)", async () => {
			...
			Enki AIEndpoint.isSelfHosted().should.be.false()
		})
	})
	...
	describe("bundled endpoints.json behavior", () => {
		...
		it("should use bundled endpoints.json when available", async () => {
			...
			await Enki AIEndpoint.initialize(bundledDir)
		...
		it("should prefer bundled endpoints.json over user file", async () => {
			...
			// Set up both configs
			await fs.writeFile(path.join(bundledDir, "endpoints.json"), JSON.stringify(bundledConfig), "utf8")
			await fs.writeFile(path.join(tempDir, ".enki", "endpoints.json"), JSON.stringify(userConfig), "utf8")
		...
		it("should throw Enki AIConfigurationError for invalid bundled file", async () => {
			const invalidConfig = {
		...
		it("should throw Enki AIConfigurationError for invalid JSON in bundled file", async () => {
