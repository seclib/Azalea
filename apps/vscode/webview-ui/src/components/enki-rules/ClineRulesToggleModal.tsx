...
const Enki AIRulesToggleModal: React.FC = () => {
	const {
		...
		hooksEnabled,
		setGlobalEnki AIRulesToggles,
		setLocalEnki AIRulesToggles,
		...
		if (isVisible) {
			FileServiceClient.refreshRules({} as EmptyRequest)
				.then((response: RefreshedRules) => {
					...
					if (response.globalEnki AIRulesToggles?.toggles) {
						setGlobalEnki AIRulesToggles(response.globalEnki AIRulesToggles.toggles)
					}
					if (response.localEnki AIRulesToggles?.toggles) {
						setLocalEnki AIRulesToggles(response.localEnki AIRulesToggles.toggles)
					}
	...
	const globalRules = Object.entries(globalEnki AIRulesToggles || {})
		.map(([path, enabled]): [string, boolean] => [path, enabled as boolean])
		.sort(([a], [b]) => a.localeCompare(b))

	// Format local rules for display with proper typing
	const localRules = Object.entries(localEnki AIRulesToggles || {})
		.map(([path, enabled]): [string, boolean] => [path, enabled as boolean])
		.sort(([a], [b]) => a.localeCompare(b))
	...
	const toggleRule = (isGlobal: boolean, rulePath: string, enabled: boolean) => {
		FileServiceClient.toggleEnki AIRule(
			ToggleEnki AIRuleRequest.create({
			...
			.catch((error) => {
				console.error("Error toggling Enki AI rule:", error)
			})
	}
	...
	const toggleWindsurfRule = (rulePath: string, enabled: boolean) => {
		FileServiceClient.toggleWindsurfRule(
			...
			.then((response: Enki AIRulesToggles) => {
				if (response.toggles) {
	...
	return (
		<div className="inline-flex min-w-0 max-w-full items-center" ref={modalRef}>
			<div className="inline-flex w-full items-center" ref={buttonRef}>
				<Tooltip>
					{!isVisible && <TooltipContent>Manage Enki AI Rules & Workflows</TooltipContent>}
					<TooltipTrigger>
						<VSCodeButton
							appearance="icon"
							aria-label={isVisible ? "Hide Enki AI Rules & Workflows" : "Show Enki AI Rules & Workflows"}
				...
				<PopupModalContainer $arrowPosition={arrowPosition} $menuPosition={menuPosition}>
					...
					<div className="flex-shrink-0 px-3 pt-2">
						...
						<div className="text-xs text-description mb-4">
							{currentView === "rules" ? (
								<p>
									Rules allow you to provide Enki AI with system-level guidance. Think of them as a persistent way
									to include context and preferences for your projects or globally for every conversation.{" "}
									<VSCodeLink
										className="text-xs"
										href="https://docs.enki.bot/features/enki-rules"
								...
								<p>
									Workflows allow you to define a series of steps to guide Enki AI through a repetitive set of
									...
									<VSCodeLink
										...
										href="https://docs.enki.bot/features/slash-commands/workflows">
										Docs
									</VSCodeLink>
								</p>
							) : currentView === "skills" ? (
								<p>
									Skills are reusable instruction sets that Enki AI can activate on-demand. When a task matches a
									skill's description, Enki AI uses the <span className="font-bold">use_skill</span> tool to load
									the full instructions.
								</p>
							) : (
								<p>
									Hooks allow you to execute custom scripts at specific points in Enki AI's execution lifecycle,
							...
							<>
								<div className="text-xs text-description mb-4">
									<p>
										...
										<VSCodeLink
											className="text-xs"
											href="https://docs.enki.bot/features/hooks"
											style={{ display: "inline", fontSize: "inherit" }}>
											Docs
										</VSCodeLink>
									</p>
								...
								{workspaceHooks.map((workspace, index) => (
									<div
										...
										<div className="text-sm font-normal mb-2">
											{workspace.workspaceName}/.enkirules/hooks/
										</div>
										<div className="flex flex-col gap-0">
											{workspace.hooks
												.sort((a, b) => a.name.localeCompare(b.name))
...
export default Enki AIRulesToggleModal
