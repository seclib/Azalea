...
const collectToolsInRange = (
	...
	startIdx: number,
	endIdx: number,
	stopCondition?: (msg: Enki AIMessage) => boolean,
...
const findCurrentApiReq = (messages: Enki AIMessage[]): { index: number; hasCost: boolean } | null => {
...
const findPrevCompletedApiReq = (messages: Enki AIMessage[], beforeIdx: number): number => {
...
export const RequestStartRow: React.FC<RequestStartRowProps> = ({
	...
	const hasCompletionResult = enkiMessages.some(
	...
	const willBeAbsorbed = useMemo(() => {
		return isApiReqAbsorbable(message.ts, enkiMessages)
	}, [message.ts, enkiMessages])
	...
	const currentActivities = useMemo(() => {
		const currentApiReq = findCurrentApiReq(enkiMessages)
		if (!currentApiReq) {
			return []
		}
		...
		if (!currentApiReq.hasCost) {
			...
			return collectToolsInRange(enkiMessages, currentApiReq.index + 1, enkiMessages.length)
		}
	...
	const hasCompletedTools = useMemo(() => {
		...
		return enkiMessages.some((msg, idx) => {
