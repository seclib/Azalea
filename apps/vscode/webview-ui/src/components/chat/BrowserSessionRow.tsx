...
import { BrowserAction, BrowserActionResult, Enki AIMessage, Enki AISayBrowserAction } from "@shared/ExtensionMessage"
	...
	const pages = useMemo(() => {
		const result: {
			currentState: {
				...
				consoleLogs?: string
				messages: Enki AIMessage[] // messages up to and including the result
			}
			nextAction?: {
				messages: Enki AIMessage[] // messages leading to next result
			}
		}[] = []

		let currentStateMessages: Enki AIMessage[] = []
		let nextActionMessages: Enki AIMessage[] = []
		...
		<BrowserSessionRowContainer style={{ marginBottom: -10 }}>
			<div style={browserSessionRowContainerInnerStyle}>
				...
				<span style={approveTextStyle}>
					{isAutoApproved ? "Enki AI is using the browser:" : "Enki AI wants to use the browser:"}
				</span>
			</div>
			<div
				style={{
