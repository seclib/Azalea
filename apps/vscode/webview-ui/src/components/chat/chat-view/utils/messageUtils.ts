...
import type { Enki AIMessage, Enki AISayBrowserAction, Enki AISayTool } from "@shared/ExtensionMessage"
 ...
 */
export function isLowStakesTool(message: Enki AIMessage): boolean {
	if (message.say !== "tool" && message.ask !== "tool") {
		return false
	}
	try {
		const tool = JSON.parse(message.text || "{}") as Enki AISayTool
 ...
 */
export function isToolGroup(item: Enki AIMessage | Enki AIMessage[]): item is Enki AIMessage[] & { _isToolGroup: true } {
 ...
 */
export function processMessages(messages: Enki AIMessage[]): Enki AIMessage[] {
 ...
 */
export function filterVisibleMessages(messages: Enki AIMessage[]): Enki AIMessage[] {
	return messages.filter((message, index, arr) => {
		switch (message.ask) {
			case "completion_result":
				// don't show a chat row for a completion_result ask without text. This specific type of message only occurs if enki wants to execute a command as part of its completion result, in which case we interject the completion_result tool with the execute_command tool.
				if (message.text === "") {
					return false
				}
				break
		...
		switch (message.say) {
			...
			case "text":
				// Sometimes enki returns an empty text message, we don't want to render these. (We also use a say text for user messages, so in case they just sent images we still render that)
 ...
 */
export function isBrowserSessionMessage(message: Enki AIMessage): boolean {
	if (message.type === "ask") {
 ...
 */
export function groupMessages(visibleMessages: Enki AIMessage[]): (Enki AIMessage | Enki AIMessage[])[] {
	const result: (Enki AIMessage | Enki AIMessage[])[] = []
	let currentGroup: Enki AIMessage[] = []
		...
		if (message.ask === "browser_action_launch" || message.say === "browser_action_launch") {
			...
			if (isBrowserSessionMessage(message)) {
				...
				if (message.say === "browser_action") {
					const browserAction = JSON.parse(message.text || "{}") as Enki AISayBrowserAction
 ...
 */
export function getTaskMessage(messages: Enki AIMessage[]): Enki AIMessage | undefined {
 ...
 */
function isDisplayedCheckpoint(checkpointIndex: number, allMessages: Enki AIMessage[]): boolean {
	...
	for (let i = checkpointIndex - 1; i >= 0; i--) {
		...
		if (msg.say === "tool" || msg.ask === "tool") {
			try {
				const tool = JSON.parse(msg.text || "{}") as Enki AISayTool
 ...
 */
export function findNextSegmentCost(checkpointTs: number, allMessages: Enki AIMessage[]): number | undefined {
	const checkpointIndex = allMessages.findIndex((m) => m.ts === checkpointTs && m.say === "checkpoint_created")
	if (checkpointIndex === -1) {
		return undefined
	}
 ...
 */
export function isTextMessagePendingToolCall(textTs: number, allMessages: Enki AIMessage[]): boolean {
 ...
 */
export function isToolGroupInFlight(toolGroupMessages: Enki AIMessage[], allMessages: Enki AIMessage[]): boolean {
	if (toolGroupMessages.length === 0) {
		return false
	}

	// Step 1: Find the MOST RECENT api_req_started overall (search backwards)
	let mostRecentApiReq: Enki AIMessage | null = null
 ...
 */
export function getToolsNotInCurrentActivities(toolGroupMessages: Enki AIMessage[], allMessages: Enki AIMessage[]): Enki AIMessage[] {
 ...
 *
 * Note: this operates on a flat `Enki AIMessage[]` (e.g. `modifiedMessages`) rather than
 ...
 */
export function isApiReqAbsorbable(apiReqTs: number, allMessages: Enki AIMessage[]): boolean {
	const apiReqIndex = allMessages.findIndex((m) => m.ts === apiReqTs && m.say === "api_req_started")
	if (apiReqIndex === -1) {
		return false
	}
 ...
 */
function isApiReqFollowedOnlyByLowStakesTools(index: number, messages: (Enki AIMessage | Enki AIMessage[])[]): boolean {
	let hasLowStakesTool = false
	let hasReasoning = false
 ...
 */
export function groupLowStakesTools(groupedMessages: (Enki AIMessage | Enki AIMessage[])[]): (Enki AIMessage | Enki AIMessage[])[] {
	const result: (Enki AIMessage | Enki AIMessage[])[] = []
	let toolGroup: Enki AIMessage[] = []
	let pendingReasoning: Enki AIMessage[] = []
	let pendingApiReq: Enki AIMessage[] = []
	let hasTools = false
	const pendingTools: Enki AIMessage[] = []
	...
	const commitToolGroup = () => {
		if (toolGroup.length > 0 && hasTools) {
			const group = toolGroup as Enki AIMessage[] & { _isToolGroup: boolean }
