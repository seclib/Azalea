...
const hoisted = vi.hoisted(() => ({
	...
	startEnki AIDeviceAuth: vi.fn(),
	completeEnki AIDeviceAuth: vi.fn(),
...
vi.mock("@enki/core", () => ({
	...
	startEnki AIDeviceAuth: hoisted.startEnki AIDeviceAuth,
	completeEnki AIDeviceAuth: hoisted.completeEnki AIDeviceAuth,
	saveLocalProviderOAuthCredentials: hoisted.saveLocalProviderOAuthCredentials,
	ProviderSettingsManager: class {},
}));

vi.mock("@enki/shared", () => ({
	getEnki AIEnvironmentConfig: () => ({ apiBaseUrl: "https://api.example" }),
}));
...
describe("onboarding auth telemetry forwarding", () => {
	beforeEach(() => {
		...
		hoisted.startEnki AIDeviceAuth.mockReset();
		hoisted.completeEnki AIDeviceAuth.mockReset();
	...
	it("forwards telemetry into completeEnki AIDeviceAuth for the device-code flow", async () => {
		hoisted.startEnki AIDeviceAuth.mockResolvedValueOnce({
			deviceCode: "dc",
			userCode: "uc",
		...
		hoisted.completeEnki AIDeviceAuth.mockResolvedValueOnce({
			access: "a",
			refresh: "r",
			expires: 0,
			accountId: "acct-1",
		...
		expect(hoisted.completeEnki AIDeviceAuth).toHaveBeenCalledTimes(1);
		const [opts] = hoisted.completeEnki AIDeviceAuth.mock.calls[0];
		expect(opts.telemetry).toBe(fakeTelemetry);
		// We do NOT forward telemetry to startEnki AIDeviceAuth — auth_started is
		// emitted by completeEnki AIDeviceAuth, so passing telemetry to the start
		// helper would double-emit the event.
		expect(hoisted.startEnki AIDeviceAuth).toHaveBeenCalledWith();
