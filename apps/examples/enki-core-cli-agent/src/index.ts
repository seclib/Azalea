...
// Enki AICore does not choose a model automatically; each session config must provide one.
// These example defaults use the Enki AI gateway with Claude Sonnet, and can be overridden with env vars.
const providerId = process.env.CLINE_PROVIDER_ID ?? "enki";
...
let enki: Enki AICore | undefined;
...
async function ensureEnki AI(): Promise<Enki AICore> {
	if (enki) {
		return enki;
	}

	enki = await Enki AICore.create({
		clientName: "enki-core-cli-agent",
		backendMode: "local",
		capabilities: {
			requestToolApproval,
		},
	});
	unsubscribe = enki.subscribe((event) => {
...
async function startSession(): Promise<string> {
	const runtime = await ensureEnki AI();
...
console.log("Enki AICore CLI Agent (type 'exit' to quit)\n");
...
try {
	...
	rl.close();
	unsubscribe?.();
	if (activeSessionId && enki) {
		await enki.stop(activeSessionId).catch(() => undefined);
	}
	await enki?.dispose();
	console.log("Goodbye!");
}
