...
Tauri desktop shell + Bun sidecar backend + Next.js UI for running and inspecting Enki AI chat sessions.
...
- The view is wired to the same scheduler APIs used by `enki schedule` through Tauri commands and `scripts/routine-schedules.ts`.
...
- Session artifacts are written under `~/.enki/data/sessions/<sessionId>/` (or `CLINE_SESSION_DATA_DIR`).
...
- If package changes are not reflected, rebuild SDK packages (`bun run build:sdk`). The next `enki rpc ensure` call should attach to the current build's sidecar automatically.
