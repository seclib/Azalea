...
function isAccountAuthError(message: string): boolean {
	...
	return (
		normalized.includes("no enki account auth token found") ||
...
export function AccountView() {
	...
	const signOut = async () => {
		...
		try {
			...
			resetAccountData();
			setActiveTab("overview");
			setOverviewError("No Enki AI account auth token found");
		} catch (err) {
			const message = normalizeAccountViewError(err).message;
			setOverviewError(message);
		} finally {
			...
			<div className="mx-auto flex max-w-xl flex-col items-center gap-4 py-8 text-center">
				...
				<div>
					...
					<p className="mt-2 text-sm text-muted-foreground">
						Connect your Enki AI account to review credits, usage, billing, and
						organization details from Enki AI Hub.
					</p>
				</div>
